...
export default function ShowRequstedUser() {
  ...
  const ApprovalManager = async (orderId) => {
    debugger;
    const response = await fetch(`https://localhost:7241/api/order/ApprovalManager`, {
      method: "PUT",
      headers: {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await fetch(
          `https://localhost:7241/api/order/getAllOrdersUser`
        );
        if (response.ok) {
          const data = await response.json();
          setOrders(data);
              ...
              <tbody>
                {orders.map((orderData, index) => (
                  <tr key={index}>
                    ...
                    <td>{orderData.firstName+" "+orderData.secondName+" "+orderData.thirdName+" "+orderData.lastName} </td>
                    ...
                    {/* <div
                      ...
                      >
                        <div className="mb-3">
                          <label className="form-lable">Masseag : </label>
                          <input
                            type="text"
                            name="socialNumber"
                      ...
                      <button
                        ...
                        onClick={() => {
                          ApprovalManager(orderData.orderId);
                        }}
                      >
                        {" "}
                        Approval
                    ...
                    <td> 
                      ...
                      >
                        <div className="mb-3">
                          <p>{orderData.messagePresident}</p>
                        </div>
                      </div>
